...
class EmployeeAdvance(Document):
    def validate(self):
        """
        Validations for Employee Advances
        """
        self.total_loan = 0
    ...
    def on_submit(self):
        """
        Employee Advances on Submit functions
        """
        gl_map = []
        fiscal_years = get_fiscal_years(self.posting_date, company='RIGPL')
        if len(fiscal_years) > 1:
            frappe.throw(f"Multiple fiscal years exist for the date {self.posting_date}. \
            	Please set company in Fiscal Year")
        else:
    ...
    def on_cancel(self):
        """
        Employee Advance on cancel functions
        """
